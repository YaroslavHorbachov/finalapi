...
const component_classes_1 = __webpack_require__(102);
const Output = new component_classes_1.SetOutputs();
const MyHistory = new component_classes_1.CreateHistory;
function httpGet(url) {
    return new Promise((resolve, reject) => {
        let xhr = new XMLHttpRequest();
        xhr.open('GET', url, true);
        xhr.onload = () => {
            let result = new component_classes_1.Message(url, xhr.getAllResponseHeaders(), xhr.status, xhr.response);
            if (xhr.status === 200) {
...
$('.form').submit(function (event) {
    Output.HideAll();
    let url = $('#url').val();
    if (!component_classes_1.Validator.isValidUrl(url)) {
        let stableUrl = url;
...
component_classes_1.Draw.jsonViewer();
...
/**Validation class*/
class Message {
    constructor(link, heads, status, body) {
        this.link = link;
        this.heads = heads;
        this.status = status;
        this.body = body;
...
Validator.expression = new RegExp(/[-a-zA-Z0-9@:%_\+.~#?&/=]{2,256}\.[a-z]{2,4}\b(\/[-a-zA-Z0-9@:%_\+.~#?&/=]*)?/gi);
exports.Validator = Validator;
class Draw {
    static isCollapsable(arg) {
